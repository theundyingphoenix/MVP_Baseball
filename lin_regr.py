...
def final_season(fn,mod_lin_regr,num):
	...
	err = round(sm.mean_squared_error(y_test,y_test_pred), 2)
...
for i in top_performer_files:
	...
	lin_regr,err_score = training_function_across_seasons(file,model_linregr,2)
	
	cont_check = float(np.std(scores))
	if cont_check <= 0.0:
		continue
	gp_war_dict[i] = line	
	mvp_dict_finder[i] = float(err_score)
